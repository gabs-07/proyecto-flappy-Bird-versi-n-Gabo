...
class _GameScreenState extends State<GameScreen>
  ...
  void _key(RawKeyEvent e, GameProvider g) {
    ...
    if (e.logicalKey == LogicalKeyboardKey.space || e.logicalKey == LogicalKeyboardKey.arrowUp) g.jump();
        ...
        child: Transform.translate(
          ...
          child: LayoutBuilder(builder: (ctx, cst) {
            ...
            return Stack(children: [
              // Fondo
              Positioned.fill(child: _Background(blend: blend, cloudAnim: _cloudCtrl)),
              // Suelo
              ...
              Positioned(left:0, right:0, top:kTopBarH, height:playH,
                child: ClipRect(child: Stack(clipBehavior: Clip.hardEdge, children: [
                  ...
                  // Obstáculos
                  if (game.isNight)
                    for (final o in game.obstacles) if (o.active) _ObstacleWidget(obs: o, W: W, H: playH),
                  // Monedas
                  for (final c in game.coins) if (!c.collected) _CoinWidget(coin: c, W: W, H: playH),
                  // Power-up items
                  for (final p in game.powerUpItems) if (!p.collected) _PowerUpWidget(item: p, W: W, H: playH),
                  // Pájaro (con skin)
                  ...
                  if (game.pendingToasts.isNotEmpty)
                    Positioned(top: 12, left: 0, right: 0,
                      child: Center(child: AchievementToast(key: ValueKey(game.pendingToasts.first.id)))),
                ]))),
              // HUD
              ...
              if (game.state == GameState.playing && game.isNight)
                Positioned(right:12, top:kTopBarH+8, child: _LevelBadge(level: game.level)),
              // Indicadores power-ups activos
              if (game.state == GameState.playing)
                Positioned(left:0, right:0, bottom:kGroundH+8,
                  child: _PowerUpIndicators(game: game)),
              // Overlays
              if (game.state == GameState.start)   const Positioned.fill(child: StartScreen()),
              if (game.state == GameState.gameOver) const Positioned.fill(child: GameOverScreen()),
...
class _ScoreDisplayState extends State<_ScoreDisplay> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Center(child: ScaleTransition(
      scale: _scale,
      child: Text('${widget.game.score}',
        style: TextStyle(
          fontSize: 72, fontWeight: FontWeight.w900,
          color: widget.game.doublePointsActive
              ? const Color(0xFFFFD700)
              : widget.game.isNight ? const Color(0xFFF5E87A) : Colors.white,
          shadows: const [Shadow(offset: Offset(2,4), blurRadius:8, color: Colors.black54)],
        )),
    ));
  }
}
...
class _SessionCoins extends StatelessWidget {
  final int count;
  const _SessionCoins({required this.count});
  @override
...
class _PowerUpIndicators extends StatelessWidget {
  final GameProvider game;
  const _PowerUpIndicators({required this.game});
  @override
  Widget build(BuildContext context) {
    return Row(
...
class _CoinWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const sz = 22.0;
    final cx = (coin.x + 1) / 2 * W;
    final cy = (coin.y + 1) / 2 * H;
...
class _PowerUpWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const sz = 34.0;
    final cx = (item.x + 1) / 2 * W;
    final cy = (item.y + 1) / 2 * H;
    ...
    return Positioned(left: cx - sz/2, top: cy - sz/2,
      child: Container(width: sz, height: sz,
        ...
        child: Center(child: Text(item.icon,
          style: TextStyle(fontSize: item.type == PowerUpType.doublePoints ? 11 : 16,
      ...
      child: Stack(alignment: Alignment.center, children: [
        ...
        if (game.shieldActive)
          Container(width: sz * 1.5, height: sz * 1.5,
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              border: Border.all(color: const Color(0xFF4488FF).withOpacity(0.6), width: 2),
              color: const Color(0xFF4488FF).withOpacity(0.12))),
        Transform.rotate(
          angle: game.bird.rotation,
          child: _buildBirdVisual(sz, skin, isDead),
        ),
      ]),
    );
  }

  Widget _buildBirdVisual(double sz, Skin skin, bool isDead) {
    return SizedBox(
      width: sz,
      height: sz,
      child: CustomPaint(
...
class _SkinBirdPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(_SkinBirdPainter o) => o.frame != frame || o.dead != dead || o.skin.id != skin.id;
}

// ─── Fondo ────────────────────────────────────────────────────
class _Background extends StatelessWidget {
  final double blend; final Animation<double> cloudAnim;
  const _Background({required this.blend, required this.cloudAnim});
  @override
  Widget build(BuildContext context) => AnimatedBuilder(
    animation: cloudAnim,
    builder: (_, __) => CustomPaint(painter: _BgPainter(blend, cloudAnim.value), child: const SizedBox.expand()));
}

Color _blendColor(Color day, Color night, double t) => Color.lerp(day, night, t)!;

class _BgPainter extends CustomPainter {
  final double blend, cloudT;
  _BgPainter(this.blend, this.cloudT);
  static const _dT = Color(0xFF38B8C8), _dB = Color(0xFFB8EAF0),
                _nT = Color(0xFF050A1E), _nB = Color(0xFF1A2A5E);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (blend < 0.95) _day(canvas, size);
    if (blend > 0.05) _night(canvas, size);
  }

  void _day(Canvas canvas, Size size) {
    final op = 1.0 - blend;
    canvas.drawCircle(Offset(size.width*0.82, size.height*0.14), 36,
      Paint()..color = const Color(0xFFFFF176).withOpacity(0.85*op));
    canvas.drawCircle(Offset(size.width*0.82, size.height*0.14), 26,
      Paint()..color = const Color(0xFFFFEE58).withOpacity(op));
    final cp = Paint()..color = Colors.white.withOpacity(0.88*op);
    for (final (rx,ry,sc) in [(0.12,0.12,1.0),(0.47,0.18,0.75),(0.74,0.08,0.85),(0.30,0.26,0.60),(0.90,0.20,0.70)]) {
  ...
  void _night(Canvas canvas, Size size) {
    final sp = Paint()..color = Colors.white.withOpacity(blend);
    for (final (rx,ry,sr) in [(0.08,0.06,1.2),(0.22,0.10,0.8),(0.38,0.04,1.0),(0.55,0.08,0.9),(0.70,0.05,1.1),(0.85,0.12,0.7),(0.95,0.07,1.0),(0.15,0.18,0.8),(0.42,0.16,0.7),(0.62,0.14,0.9),(0.78,0.20,0.8),(0.30,0.22,0.6),(0.50,0.24,0.7),(0.88,0.26,0.9),(0.10,0.30,0.5),(0.65,0.28,0.6)]) {
      canvas.drawCircle(Offset(rx*size.width, ry*size.height*0.8), sr*blend, sp);
    }
    canvas.drawCircle(Offset(size.width*0.80, size.height*0.16), 30,
      Paint()..color = const Color(0xFFF5E87A).withOpacity(0.92*blend));
  ...
  @override bool shouldRepaint(_BgPainter o) => o.blend != blend || o.cloudT != cloudT;
}
...
class _GroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = -(off%sp); x < size.width+sp; x+=sp) canvas.drawLine(Offset(x,2),Offset(x-4,20),bp);
...
class _PipeWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final prefix = isNight ? 'night/' : '';
    final topImg = 'assets/images/${prefix}pipe_top${isNight?"_night":""}.png';
    final botImg = 'assets/images/${prefix}pipe_bottom${isNight?"_night":""}.png';
    final capImg = 'assets/images/pipe_cap.png';
    ...
    Widget im(String path, double w, double h, Color fb) =>
      Image.asset(path, width:w, height:h, fit:BoxFit.fill, errorBuilder:(_,__,___)=>Container(color:fb));
    return Positioned(left:left, top:0, child:SizedBox(width:pW, height:H,
      child:Column(children:[
        SizedBox(height:(topH-capH).clamp(0.0,H), child:im(topImg,pW,topH-capH,bodyC)),
        if(capH>0) SizedBox(height:capH, child:im(capImg,pW*1.12,capH,capC)),
        SizedBox(height:hG*2),
        if(capH>0) SizedBox(height:capH, child:im(capImg,pW*1.12,capH,capC)),
        SizedBox(height:(botH-capH).clamp(0.0,H), child:im(botImg,pW,botH-capH,bodyC)),
      ])));
  }
}
...
class _MeteorP extends CustomPainter {
  @override void paint(Canvas canvas, Size s) {
    final cx=s.width/2, cy=s.height*0.4;
    for(int i=0;i<8;i++){final r=(8-i)*2.5;canvas.drawCircle(Offset(cx,cy+i*5.0),r.toDouble(),Paint()..color=Color.lerp(const Color(0xFFFF6600),Colors.transparent,i/8)!);}
    canvas.drawCircle(Offset(cx,cy),22,Paint()..color=const Color(0xFF5A5A7A));
    canvas.drawCircle(Offset(cx,cy),22,Paint()..color=const Color(0xFFFF6A00)..style=PaintingStyle.stroke..strokeWidth=2);
  }
  @override bool shouldRepaint(_)=>false;
}
class _BatP extends CustomPainter {
  @override void paint(Canvas canvas, Size s) {
    final cx=s.width/2,cy=s.height*0.6;
    final wp=Paint()..color=const Color(0xFF3A1060);
    canvas.drawPath(Path()..moveTo(cx,cy)..quadraticBezierTo(cx-30,cy-28,cx-44,cy-14)..quadraticBezierTo(cx-38,cy+2,cx,cy+4),wp);
    canvas.drawPath(Path()..moveTo(cx,cy)..quadraticBezierTo(cx+30,cy-28,cx+44,cy-14)..quadraticBezierTo(cx+38,cy+2,cx,cy+4),wp);
    canvas.drawOval(Rect.fromCenter(center:Offset(cx,cy),width:28,height:22),Paint()..color=const Color(0xFF2A0848));
    canvas.drawOval(Rect.fromCenter(center:Offset(cx,cy-14),width:22,height:18),Paint()..color=const Color(0xFF2A0848));
    for(final ex in [cx-6.0,cx+6.0]){canvas.drawCircle(Offset(ex,cy-14),4,Paint()..color=const Color(0xFFFF0000));canvas.drawCircle(Offset(ex,cy-14),2,Paint()..color=const Color(0xFFFF8888));}
  }
  @override bool shouldRepaint(_)=>false;
}
class _LaserP extends CustomPainter {
  final bool on; _LaserP({required this.on});
  @override void paint(Canvas canvas, Size s) {
    for(final ex in [20.0,s.width-20]){canvas.drawCircle(Offset(ex,s.height*0.35),on?9.0:6.0,Paint()..color=on?const Color(0xFFFF2020):const Color(0xFF440000));if(on)canvas.drawCircle(Offset(ex,s.height*0.35),5,Paint()..color=const Color(0xFFFF8888));}
    if(on){canvas.drawRect(Rect.fromLTWH(20,s.height*0.31,s.width-40,3),Paint()..color=const Color(0xFFFF4444));canvas.drawRect(Rect.fromLTWH(20,s.height*0.63,s.width-40,3),Paint()..color=const Color(0xFFFF4444));}
  }
  @override bool shouldRepaint(_LaserP o)=>o.on!=on;
}
class _UfoP extends CustomPainter {
  @override void paint(Canvas canvas, Size s) {
    final cx=s.width/2,cy=s.height*0.38;
    canvas.drawPath(Path()..moveTo(cx-28,cy+10)..lineTo(cx+28,cy+10)..lineTo(cx+44,s.height)..lineTo(cx-44,s.height)..close(),Paint()..color=const Color(0x2244FFAA));
    canvas.drawOval(Rect.fromCenter(center:Offset(cx,cy+6),width:80,height:20),Paint()..color=const Color(0xFF404858));
    canvas.drawOval(Rect.fromCenter(center:Offset(cx,cy-4),width:38,height:28),Paint()..color=const Color(0xFF303848));
    canvas.drawOval(Rect.fromCenter(center:Offset(cx,cy-6),width:26,height:18),Paint()..color=const Color(0xFF44DDFF).withOpacity(0.6));
    final lc=[const Color(0xFFFF4444),const Color(0xFFFFFF44),const Color(0xFF44FF44),const Color(0xFF44FFFF),const Color(0xFF4444FF)];
    for(int i=0;i<5;i++)canvas.drawCircle(Offset(cx-20+i*10,cy+6),3,Paint()..color=lc[i]);
  }
  @override bool shouldRepaint(_)=>false;
}
class _GhostP extends CustomPainter {
  @override void paint(Canvas canvas, Size s) {
    final cx=s.width/2;
    final path=Path()..moveTo(cx-22,s.height)..lineTo(cx-22,s.height*0.35)..quadraticBezierTo(cx,s.height*0.1,cx+22,s.height*0.35)..lineTo(cx+22,s.height)..quadraticBezierTo(cx+14,s.height-10,cx+6,s.height)..quadraticBezierTo(cx,s.height-14,cx-6,s.height)..quadraticBezierTo(cx-14,s.height-10,cx-22,s.height)..close();
    canvas.drawPath(path,Paint()..color=const Color(0xBBCCCCFF));
    for(final ex in [cx-8.0,cx+8.0]){canvas.drawCircle(Offset(ex,s.height*0.45),7,Paint()..color=const Color(0xFFFFFF44));canvas.drawCircle(Offset(ex,s.height*0.45),4,Paint()..color=const Color(0xFFFF8800));canvas.drawCircle(Offset(ex,s.height*0.45),2,Paint()..color=Colors.black);}
  }
  @override bool shouldRepaint(_)=>false;
}
...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(color: Colors.black.withOpacity(0.28),
...
class _LevelBadge extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: const Color(0xFF1A2A5E).withOpacity(0.85), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFF5E87A).withOpacity(0.6))),
    child: Text('Nv. $level', style: const TextStyle(color: Color(0xFFF5E87A), fontSize: 12, fontWeight: FontWeight.bold)));
}
...
class _PauseOverlay extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(
    ...
    child: const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
      Icon(Icons.pause_circle_filled_rounded, color: Colors.white, size: 72),
      SizedBox(height: 12),
      Text('PAUSADO', style: TextStyle(fontSize: 36, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 4)),
      SizedBox(height: 8),
      Text('Toca o presiona P para continuar', style: TextStyle(color: Colors.white70, fontSize: 14)),
    ])));
}
